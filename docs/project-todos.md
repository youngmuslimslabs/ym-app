...
- **Part-1 typeform motion & transitions (Typeform-style).** Add smooth question-to-question motion to the new `/onboarding-v2` flow: slide/fade between steps (directional on advance vs. Back), progress-bar easing, and input entrance/focus micro-interactions — matching Typeform's feel. Use the `frontend-design` skill; respect `prefers-reduced-motion`. (Distinct from the old 7-step onboarding motion item above, which targets the retiring flow.)
