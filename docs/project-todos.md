...
30. **[P2] Rework the full NN hierarchy with Zaid Khan** — the `00020` seed took the Fundraising master list mostly verbatim, but the source mixes geography with *status* ("Expansion" was used as a Region, and the same subregions — NY West, Pennsylvania — appeared under both their real region and "Expansion"). For launch we folded expansion areas into real active regions (created **DMV** for VA/MD/WV; **West** for WA + the LV/AZ/LA bucket; Minnesota→Midwest) and treated them as normal regions. This is a stopgap. Sit with **Zaid Khan** to define the *canonical* hierarchy end-to-end: real region boundaries (does WV belong in DMV? should "Expansion" be a status flag instead of folded?), subregion naming (`Dallas Expansions`, the region+subregion both named `West`), and NN-level cleanup (terse codes `LV`/`AZ`/`LA`, numeric NN names `571`/`75`). Then migrate to the corrected structure.
