...
- [x] Bundle size check — `scripts/check-bundle-size.ts` compares **gzipped** per-route chunk sizes against `bundle-size-baseline.json` (fails >10%, warns >5%). Baseline auto-regenerated by `.github/workflows/bundle-baseline.yml` on push to main; refresh manually with `bun run bundle:baseline`. PR CI uses `--check-only` so PRs cannot silently bootstrap a baseline.
- [ ] Bundle gate `continue-on-error` removal — set during bootstrap until `bundle-baseline.yml` runs once on main; remove from `.github/workflows/ci.yml` afterward.
