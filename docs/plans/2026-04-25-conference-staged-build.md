...
- **Mobile drag-to-dismiss visual feedback.** ✅ Implemented in commit `b876856` (`feat(ui): bottom sheet drag-to-dismiss visual feedback`). Shared hook `useBottomSheetDragToDismiss` lives in `src/hooks/use-bottom-sheet-drag.ts`; applied to `SessionSheet.tsx`, `RosterSheet.tsx`, and `SessionCommentsSheet.tsx`. Sheet tracks the finger via direct DOM transform during `touchmove`, snaps back via inline transition below 60px, and on dismiss leaves the inline transform in place so Radix's exit keyframe (`slide-out-to-bottom`, no `from`) picks up from the finger's last position.
  ...
  - [ ] **Extract `<DragHandle>` component.** Three sheets (`SessionSheet`, `RosterSheet`, `SessionCommentsSheet`) now share byte-identical drag-handle JSX (`<div className="flex justify-center pt-2 pb-1 shrink-0 touch-none" {...dragHandleProps} aria-hidden="true"><div className="h-1 w-10 rounded-full bg-border" /></div>`). Per CLAUDE.md's "reuse is a component" rule, second-caller threshold has been crossed twice. Either extract a `<DragHandle {...dragHandleProps} />` component or have `useBottomSheetDragToDismiss` return a `dragHandle` JSX element. Do before a fourth bottom sheet appears.

- **DST spring-forward bug in `composeTzIso`.** `src/app/(app)/admin/conferences/lib/datetime.ts` derives the timezone offset from the input UTC instant rather than the target wall-clock instant, so wall clocks in the post-transition / pre-transition-UTC window come out 1h off. Concrete: `composeTzIso('2026-03-08', '03:30', 'America/New_York')` returns `2026-03-08T08:30:00.000Z`; the correct value is `07:30`. Affected window: ~02:00–07:00 wall clock on the second Sunday of March, US tz only — vanishingly rare for an admin to schedule a session there, but not zero. A pinned `it.skip` lives in `datetime.test.ts` that lights up once this is fixed. Fix sketch: iterate (compute `O` at `W`, then recompute `O` at `W - O` and use that), or pull in `date-fns-tz` and delete the hand-roll.

- **Tier 2 unit tests for conferences server actions.** `client-actions.ts` files in `app/(app)/conferences/[conferenceId]/` and `app/(app)/admin/conferences/` encode the conference business rules (signup capacity, time-gating, check-in, feedback-window, publish lifecycle) and have ~290 lines of branch-rich logic with zero unit tests. Mock the Supabase client and write ~30–40 tests across the actions. Goal: lift global coverage above the original `{lines: 4, branches: 6, functions: 4, statements: 4}` floors so the rebaselined `3` floor in `vitest.config.mts` can ratchet back up. Estimated 30–60 min for one well-bounded action (`submitFeedback` is the smallest target).
