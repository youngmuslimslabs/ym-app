...
39. **[P2] Harden `role_assignments` DELETE RLS against self-deleting system roles** `[AUDIT]` — the DELETE policy only checked ownership (`user_id = get_current_user_id()`), so a user could delete their own admin-granted **system** role (e.g. Event Admin) via a direct API call; INSERT/UPDATE already block self-managing system roles. Baseline hardened in `00001_initial_schema.sql` (same `NOT EXISTS (… category='system')` guard added to the DELETE `USING`) — **PR #47**. **Still pending: apply to remote** via native `psql` (DROP + CREATE POLICY; see the `ym-db-changes` skill). Client-side backstop already live in prod (`writableRoles()` filtering, PR #45); this is the DB-level hard guard. No type regen needed (RLS policies don't affect generated types).
...
- **2026-07-11 session check-in grace widened to 60 min** ✅ **[DONE]** — `GRACE_MINUTES` in `src/app/(app)/conferences/[conferenceId]/lib/checkInWindow.ts` bumped from 15 → 60, and the mirrored server-side `interval '15 minutes'` in `signup_for_session` / `cancel_signup` bumped alongside (baseline `00001_initial_schema.sql` updated for greenfield replay; incremental `supabase/migrations/00023_grace_60_minutes.sql` `CREATE OR REPLACE`s both functions for the live DB). No mechanic change — same removability invariant, same error surface, just a longer tail. Unit + component tests updated to the new 16:00 boundary. Remote migration still needs to be applied (`psql --single-transaction -f 00023_grace_60_minutes.sql`).
- **2026-07-11 session check-in grace period + feedback gating (issue #53)** ✅ **[DONE + VERIFIED LIVE]** — check-in now stays open for the whole session **plus a 15-min grace tail** after `end_at` (was: closed instantly at end); feedback opens at session end and only for checked-in attendees; checking in during the tail flips the sheet straight into the feedback form (same motion). State machine centralized in `lib/checkInWindow.ts` (`GRACE_MINUTES=15`, `getSessionState` → `{window, slot}`) consumed by both `SessionCard` and `SessionSheet` so they can't disagree; card chrome follows "bright while an action is wanted" (grace nudge + pending feedback stay active; dims once nothing is left to ask). **Enforcement is app-layer by explicit owner decision** (internal app, no bypass payoff; a drafted DB migration — RPC window gate + feedback RLS dependency — was deliberately dropped; the pre-existing `end_at < now()` feedback RLS gate remains). High-effort review: 6 findings, 5 fixed (incl. a sticky latch so a 60s clock tick can't unmount a half-typed code), 1 accepted (clock-skew feedback-RLS edge at `end_at`, self-correcting). **Verified end-to-end against the real app + dev DB** (Playwright, disposable magic-link user): wrong code rejected, grace check-in → same-motion feedback → rating persisted through RLS, missed session locked, live session's feedback correctly held until end; test data fully deleted after. 333 tests pass. Design doc: `docs/plans/2026-07-11-session-checkin-grace-feedback-design.md`.
- **2026-07-11 mobile overlay modals** ✅ **[DONE — code, pending visual QA + 1 data fix]** (branch `fix/mobile-overlay-modals`, design at `docs/plans/2026-07-11-mobile-overlay-modals-design.md`) — fixes issues **#59** (onboarding roles dropdown glitchy), **#51** (DOB date picker cut off), **#63** (profile project-role-type dropdown off-screen). Root cause: every affected overlay is a Radix primitive *anchored to its trigger*; on ≤393px viewports the collision math clips/scroll-locks it. Fix is a hybrid gated on the existing `useIsMobile()` (≤767px), **desktop unchanged**: `SearchableCombobox` → bottom `Sheet` on mobile (keyboard-safe, `max-h-[85dvh]`, search pinned top); `DatePicker` → centered `Dialog` on mobile (`max-w-[calc(100vw-2rem)]`, now also closes on select); new **`ResponsiveSelect`** (`src/components/responsive-select.tsx`) → centered `Dialog` option list on mobile, wired into onboarding `SelectStep` (also fixes subregion + neighbornet steps). Bonus: fixed a latent jsdom gap — `vitest.setup.ts` now stubs `window.matchMedia` so `useIsMobile()` consumers default to desktop in tests. Full gate green locally: **307 tests, tsc clean, lint 0 errors, build OK**. New component tests cover both mobile/desktop branches. **Still owed:** real-device visual QA at 329/375/393/430px — **top check: on a phone, pick a role in onboarding and confirm the next step is tappable** (review flagged a possible stuck-body if unmounting the open Radix modal on auto-advance; mechanism analysis says hard-unmount cleans up synchronously and jsdom agrees, but confirm on real mobile Safari). **Decided not to do:** #51's secondary AC to remove/fix Shaheer Ghazenfer's DOB — the correct value is unknown, so nulling or guessing it is worse than leaving it (owner call, 2026-07-11). **High-effort code review applied** (2026-07-11): fixed a11y listbox/option roles + scroll-selected-into-view on `ResponsiveSelect`, reset combobox search on dismiss, and cleared the Sheet Close-button/search overlap. Accepted as known tradeoff: 1-frame desktop flash from `useIsMobile()` (matches existing sidebar). **Deferred follow-up:** extract a shared responsive-overlay primitive to replace the branch hand-rolled 3× across ResponsiveSelect/DatePicker/SearchableCombobox — revisit once the pattern stops changing.
- **2026-07-09 login button centering + post-login lag** ✅ **[DONE + VERIFIED LIVE]** (PR #43 merged, git `ecafbec`) — intermittent off-center "Continue with Google" button (mobile + desktop): Google Identity Services bakes a fixed pixel width into its button iframe at render time; the button was rendered with no explicit `width`, so it inherited whatever the container measured at that instant, and centering leaned on fragile `[&>div]` descendant selectors assuming a GIS DOM structure GIS doesn't guarantee. Fix (`GoogleSignInButton.tsx`): render at the container's measured width (clamped to the 400px GIS max) so the iframe fills the slot, drop the descendant CSS for a plain centered flex, and re-render via `ResizeObserver` only when the integer width changes (fixes the render-before-layout race + rotation/resize). Post-login "stuck on login" lag: login page used `getUser()` (network token re-validation) before navigating → switched to `getSession()` (local read); and `GoogleSignInButton` fired `onSuccess` without `await` and cleared `isLoading` in `finally`, so "Signing in…" vanished while the redirect round-trips were still running → now awaits `onSuccess` and holds the loading state until navigation. **Verified end-to-end** against the real Google OAuth client (chrome-devtools on a local build): GIS iframe rendered with explicit `width=334`, symmetric gaps, stable across reloads, re-renders on container resize; sign-in network log showed the `/auth/v1/user` round-trip gone and a clean `/login → /home` single hop. High-effort code review applied: memoized the LoginPage callbacks (fixes a confirmed GIS re-init/flicker on parent re-render), reset `isRedirecting` on the throw path, `onSuccess` now returns whether it navigated (so the button can't be stranded on a spinner), and a no-`ResizeObserver` fallback renders the button without an explicit width. Regression tests added for both files (226 tests pass). **Live on prod** — deploy `6a5061ac` (`ready`, secret scan clean).
- **2026-07-09 Netlify secret-scan false positive** ✅ **[DONE]** (PR #44 merged, git `0c06fb7`) — deploy builds were failing secret scanning because `GOOGLE_ADMIN_EMAIL`'s value (an `@youngmuslims.com` email) appears in `supabase/seed/ymlc-2026.sql` as a conference roster row (the admin is also an attendee). Seed SQL is a DB fixture, not a deployed artifact (publish dir is `.next`), so it's a false positive. Fix: `SECRETS_SCAN_OMIT_PATHS = "supabase/**"` in `netlify.toml` — DB source excluded, app code + build output still scanned. This had silently broken the prod deploy pipeline (prod was serving an older build); resolved and confirmed with a clean production deploy.
