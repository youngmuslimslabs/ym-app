...
- **[CABINET CLUSTER — deferred until the Cabinet feature is built] Structure, naming, leadership, and seed.** Both `departments` and `teams` are currently **empty** (0 rows) and unreferenced in app code (only in generated types), so none of this is July-critical. Decisions (resolved this session — do not relitigate):
    - **Two-table structure stays** — keep `departments` and `teams` as separate tables (two-level hierarchy: department heads *and* team heads). Not a single table with a department enum — a head needs a row to be head of.
    - **Leadership + rosters are `role_assignments`, not columns.** The seed already defines the exact roles: **Cabinet Team Lead** (`scope_type=team`, max 1), **Cabinet Team Member** (`scope_type=team`, unlimited), **Department Head** (`scope_type=department`, max 1). "Who leads team X" / "team X roster" = role_assignments with `scope_id` = the team id. So **no** `team_leader`/`team_members`/`department_head` scalar columns, and **no `team_members` junction table** — rosters are already the `Cabinet Team Member` role.
    - **Rename for specificity (execute WITH this work, not before):** tables `departments`→`cabinet_departments`, `teams`→`cabinet_teams`; `scope_type` enum values `department`→`cabinet_department`, `team`→`cabinet_team` (keep the convention that `scope_type` value = singular table name); role **"Department Head"→"Cabinet Department Head"** (parallel with Cabinet Team Lead/Member). Leave geography names (`region`/`subregion`/`neighbor_net`) unchanged — already specific. Low app-code risk (tables unused), but the `scope_type` values are referenced by live seeded `role_types`, so it's a real migration + type regen, not a no-op.
    - **[ACTION when org data available]** Get the real Cabinet department → team structure from the owner and fold it into the seed alongside the geography seed (P0 #1 / `00004_seed_data.sql`), using the final `cabinet_*` names. *Blocked on: owner providing the real department/team list.*
