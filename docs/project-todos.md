...
- **2026-07-06 migration squash** — collapsed `00001`–`00019` + the stale `_run_all.sql` into a single `supabase/migrations/00001_initial_schema.sql` baseline (reconstructed from live catalog introspection) + `supabase/seed.sql` (role_types only). Folded in the **cabinet renames** (`cabinet_departments`/`cabinet_teams`, `scope_type` `cabinet_department`/`cabinet_team`, "Cabinet Department Head") and the **approved additive changes** (conferences `scope_level`/`region_id`/`subregion_id`/`point_of_contact_user_id` + hierarchy CHECK, `memberships.subregion_id` + relaxed "at most one location" CHECK, `is_expansion` on regions/subregions/neighbor_nets). Dropped a dead `set_updated_at` function. ⚠️ **NOT yet replay-verified** — Docker was down at squash time, so run `supabase db reset` on a local stack for a zero-error replay, then regenerate `src/types/database.types.ts`, before the prod wipe/rebuild. Remote still carries the old `00001`–`00019` ledger (diverges by design until the wipe — do **not** `db push` until then).
