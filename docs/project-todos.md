...
6. **[P0] Production OAuth + Google authorized origins** `[AUDIT]` — swap in prod Google OAuth credentials. Because login uses `signInWithIdToken` (Google Identity Services), the cutover dependency is the **Authorized JavaScript Origins** on the Google Cloud OAuth client (add `https://app.youngmuslims.com`), plus the Supabase Auth **Site URL** — not just redirect URIs. **Still open (2026-06-30):** the JS-origin add on the *prod* OAuth client is the last thing gating live login (Supabase Site URL/redirect already set — see #7).

7. **[P0] Custom domain — `app.youngmuslims.com` on Netlify** — **Decided:** single host **Netlify**; subdomain **`app.`** (NOT the apex — apex keeps forwarding to the ymsite.com marketing site, isolating the change from the Workspace `MX` records that carry `@youngmuslims.com` email). Domain is registered + DNS-managed at **Squarespace** (Workspace set up via Squarespace → MX auto-managed under a separate "Google records" section, untouchable from the custom-records panel). ✅ **Done 2026-06-30:** Squarespace CNAME `app → dev-ym-app.netlify.app` (TTL 5m); Netlify domain added, set primary, **Let's Encrypt SSL issued**; Supabase Auth Site URL + Redirect URL = `https://app.youngmuslims.com`. **Remaining:** (a) Google Cloud **Authorized JavaScript origins** += `https://app.youngmuslims.com` on the *prod* OAuth client — **the one that actually breaks login if missed** (folds into #6); (b) confirm the three `NEXT_PUBLIC_*` vars are set on the Netlify site; (c) end-to-end sign-in → onboarding → directory test on the live domain (#10). **Corrections to the earlier audit (verified):** login is GIS `signInWithIdToken`, **not** an OAuth-redirect flow → `redirect_uri_mismatch` / `trailingSlash` / manifest `start_url` concerns are **moot**, and `/auth/callback` is **dead code**; `netlify.toml` `publish = ".next"` is **correct** for the Next.js Runtime v5 (OpenNext) — no change; the "both Netlify *and* Vercel build every PR, confirmed in CI" claim was **inaccurate** (`ci.yml` has no deploy step — any dual-build is the Vercel GitHub App; kill it).
